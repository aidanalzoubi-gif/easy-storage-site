...
export function StepStorage({ onNext, onBack }: StepStorageProps) {
  ...
  const totalPrice = calculateStoragePrice(totalBoxes);

  const updateFurnitureQuantity = (type: string, boxEquivalent: number, delta: number) => {
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    = {furniture.boxEquivalent} {furniture.boxEquivalent === 1 ? 'box' : 'boxes'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="h-8 w-8"
                    onClick={() =>
                      updateFurnitureQuantity(furniture.type, furniture.boxEquivalent, -1)
                    }
                    disabled={quantity <= 0}
                  >
