...
export function StepPayment({ onSubmit, onBack, isSubmitting }: StepPaymentProps) {
  ...
  const totalPrice = calculateTotalPrice(totalBoxes, hasInsurance);
        ...
        <div className="mt-4 space-y-3">
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">
              Storage ({totalBoxes} {totalBoxes === 1 ? 'box' : 'boxes'})
            </span>
            <span className="text-foreground">
              {formatPrice(totalBoxes * 14.99)}
            </span>
          </div>
          {hasInsurance && (
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Insurance</span>
              <span className="text-foreground">{formatPrice(29)}</span>
            </div>
          )}
        ...
        <div className="rounded-xl border border-border bg-muted/30 p-6">
          {paymentMethod === 'cash' && (
            <p className="text-muted-foreground">
              You&apos;ll pay the deposit in cash when we pick up your items. Please have{' '}
